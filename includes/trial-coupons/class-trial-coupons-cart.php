...
class WCST_Trial_Coupons_Cart {
	...
	private function debug( $message ) {
		...
		wc_get_logger()->info( (string) $message, [ 'source' => 'navest-trial-coupons' ] );
	}
	...
	public function custom_error_message( $err, $err_code, $coupon ) {
		if ( isset( $coupon->error_code ) && 'wcst_no_subscription_in_cart' === $coupon->error_code ) {
			return __( 'This coupon can only be applied when the cart contains a subscription product.', 'navest-toolkit-for-woocommerce' );
		}
		return $err;
	}
	...
	public function cart_coupon_label( $label, $coupon ) {
		if ( $coupon instanceof WC_Coupon && $coupon->is_type( WCST_TRIAL_COUPON_TYPE ) ) {
			return esc_html__( 'Free trial', 'navest-toolkit-for-woocommerce' ) . ' (' . esc_html( strtoupper( $coupon->get_code() ) ) . ')';
		}
		return $label;
	}
	...
	public function cart_discount_amount_html( $discount_html, $coupon ) {
		...
		if ( $length <= 0 || '' === $period ) {
			return esc_html__( 'Free trial period', 'navest-toolkit-for-woocommerce' );
		}
	...
	private function format_trial_period( $length, $period ) {
		switch ( $period ) {
			case 'day':
				...
				return sprintf( _n( '%d day', '%d days', $length, 'navest-toolkit-for-woocommerce' ), $length );
			case 'week':
				/* translators: %d: number of weeks */
				return sprintf( _n( '%d week', '%d weeks', $length, 'navest-toolkit-for-woocommerce' ), $length );
			case 'month':
				/* translators: %d: number of months */
				return sprintf( _n( '%d month', '%d months', $length, 'navest-toolkit-for-woocommerce' ), $length );
			case 'year':
				/* translators: %d: number of years */
				return sprintf( _n( '%d year', '%d years', $length, 'navest-toolkit-for-woocommerce' ), $length );
			default:
